...
public class SceneFader : MonoBehaviour
{
    ...
    public void Awake()
    {
        ...
        {
            Instance = this;
            // DontDestroyOnLoad(gameObject); // 이전 코드에서 제거되었으므로 유지하지 않습니다.
        }
        else
        {
            Destroy(gameObject);
    ...
    {
        // 💡 [수정] 씬 로드 직후 검은 화면과 텍스트의 알파를 1.0으로 강제 설정
        if (blackScreen != null)
        {
            Color c = blackScreen.color;
            c.a = 1f; 
