...
public class SceneFader : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (Instance == null)
        {
            ...
            // 💡 [수정] blackScreen UI 요소가 씬 이동 시 파괴되는 것을 방지하기 위해 
            // blackScreen의 최상위 Canvas 오브젝트를 DontDestroyOnLoad로 설정합니다.
            if (blackScreen != null)
            {
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        if (blackScreen != null)
        {
            // 💡 씬 로드 직후, 페이드 인 시작 전 Alpha를 1.0으로 강제 설정
            Color c = blackScreen.color;
            c.a = 1f; 
            blackScreen.color = c;
        }
        ...
        if (player != null) {
            ...
            if (rb != null)
            { 
                // 🛑 [수정] linearVelocity 대신 velocity를 사용
    ...
    private IEnumerator FadeIn()
    {
        ...
        // 씬 로드 직후 검은 화면 상태(Alpha 1)에서 시작합니다. (OnSceneLoaded에서 이미 설정됨)
