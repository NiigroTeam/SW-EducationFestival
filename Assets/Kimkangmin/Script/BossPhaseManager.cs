...
public class BossPhaseManager : MonoBehaviour
{
    ...
    // 🌟 수정: Slider 대신 Image UI로 변경
    [Tooltip("씬에 배치된 보스 HP Image UI (Type: Filled)")]
    ...
    void Start()
    {
        ...
        // 3. HP Image 초기 설정 및 가시성 제어
        if (bossHPImage != null)
        {
            if (snakeHealth != null)
            {
                bossHPImage.gameObject.SetActive(true); 
                
                // 🌟 수정: Health 스크립트의 hpImage 필드에 연결
                snakeHealth.hpImage = bossHPImage;
            }
            else
            {
                bossHPImage.gameObject.SetActive(false); 
            }
        }
    }
    ...
    public void OnSnakeKilled()
    {
        ...
        if (turtleHealth != null && bossHPImage != null)
        {
            // 🌟 수정: 현무 Health 스크립트의 hpImage 필드에 연결
            turtleHealth.hpImage = bossHPImage;
            
            // 현무의 HP를 최대치로 설정 (Health 스크립트가 DOTween으로 부드럽게 업데이트)
            turtleHealth.currentHP = turtleHealth.maxHP;
            
            // 🌟 수정: Image.fillAmount를 직접 설정하여 즉시 최대치로 보이게 함
            bossHPImage.fillAmount = turtleHealth.currentHP / turtleHealth.maxHP;
        }
        ...
        if (bossMoveTarget != null && player != null && mainCamera != null)
        {
            ...
            // 카메라 Z축을 유지하며 이동
            Vector3 camTargetPos = bossMoveTarget.position;
            camTargetPos.z = mainCamera.transform.position.z;
